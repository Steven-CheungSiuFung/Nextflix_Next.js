...
export async function getFavouritedVideoId(token, userId) {
  const operationsDoc = `
  query getFavouritedVideoId($userId: String!) {
...
`;
  ...
  const response = await queryHasuraGQL(
    operationsDoc,
    "getFavouritedVideoId",
    { userId },
    token
  );

  if (response.errors) {
    throw new Error("getFavouritedVideoId Error", {
      message: response?.errors[0]?.message,
    });
  }
